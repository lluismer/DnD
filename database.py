...
def add_monster_to_db(char: Character):
    con = sqlite3.connect("database/monsters.db")
    cur = con.cursor()

    cur.execute("""CREATE TABLE IF NOT EXISTS monsters(name TEXT PRIMARY KEY, hp INTEGER, full_data JSON)""")

    data_dict = asdict(char)

    cur.execute(
        "INSERT OR REPLACE INTO monsters(name,hp,full_data) VALUES (?, ?, ?)",
        (char.name,char.hp,json.dumps(data_dict))
    )
...
def remove_monster():
    ...
    cur = con.cursor()
    cur.execute(
        "DELETE FROM monsters WHERE name = ?",
        (name,)
    )
